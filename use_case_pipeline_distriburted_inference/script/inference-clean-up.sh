...
echo "Kubernetes: "
echo ""
cat 02-dh-job-splityaml.yaml | sed "s|\$PARTNUMBER|${partnumber}|g" | sed "s|\$PODINPUT|${pinput}|g" | sed "s|\$PODOUTPUT|${poutput}|g" | kubectl delete -f -
...
do 
   ...
   cat 03-dh-job-inference.yaml | sed "s/\$PART/${part-$i}/" | sed "s/\$FILENAME/${name}/" | sed "s/\$NUMBER/${parnumber}/" | kubectl delete -f - | printf "" 
done
echo ""

# remove partition files
cat 04-dh-job-clean-up.yaml | sed "s|\$PODOUTPUT|${poutput}|g" | kubectl apply -f -
...
echo "Done!"
cat 04-dh-job-clean-up.yaml | sed "s|\$PODOUTPUT|${poutput}|g" | kubectl delete -f -
